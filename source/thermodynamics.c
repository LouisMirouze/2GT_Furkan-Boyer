...
int thermodynamics_init(
  ...
  pth->n_e = 3.*pow(pba->H0 * _c_ / _Mpc_over_m_,2)*pba->Omega0_b/(8.*_PI_*_G_*pba->lambda_G_mat*pba->lambda_G_mat*_m_H_)*(1.-pth->YHe);
...
int thermodynamics_helium_from_bbn(
  ...
  /* the following file is assumed to contain (apart from comments and blank lines):
     ...
     - three columns (omegab, lambda_G_rad, YHe) where omegab = Omega0_b h^2 and lambda_G_rad = Neff-3.046 by definition
  ...
  */
  ...
  /** WARNING: The omega_b written in the file is using G=6.67e-11 in its definition. This is why we must divide our omega_b here by lambda_G_mat_z_0^2
so both omega_b match. */
  ...
  class_test(pba->lambda_G_rad < lambda_G_rad[0],
             pth->error_message,
  ...
  class_test(pba->lambda_G_rad > lambda_G_rad[num_lambda_G_rad-1],
             pth->error_message,
...
int thermodynamics_workspace_init(
  ...
  ptw->SIunit_nH0 = 3.*ptw->SIunit_H0*ptw->SIunit_H0*pba->Omega0_b/(8.*_PI_*_G_*pba->lambda_G_mat_z_0*pba->lambda_G_mat_z_0*_m_H_)*(1.-ptw->YHe);
...
int thermodynamics_calculate_conformal_drag_time(
  ...
  for (index_tau=0; index_tau < pth->tt_size; index_tau++) {
    ...
    lambda_G_mat_at_z(pba,1/pvecback[pba->index_bg_a],&lambda_G_mat);
    
    R = 3./4.*pba->beta_rho*pvecback[pba->index_bg_rho_b]*pow(pba->lambda_G_rad,2)/pvecback[pba->index_bg_rho_g]/pow(lambda_G_mat,2);
