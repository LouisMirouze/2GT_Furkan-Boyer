...
int background_functions(
  ...
  lambda_G_mat_at_z(pba,1/a-1,&lambda_G_mat);
  ...
  /* baryons */
  pvecback[pba->index_bg_rho_b] = pba->Omega0_b * pow(pba->H0,2) * pow(lambda_G_mat/pba->lambda_G_mat_z_0,2) / pow(a,3);
  ...
  if (pba->has_cdm == _TRUE_) {
    pvecback[pba->index_bg_rho_cdm] = pba->Omega0_cdm * pow(pba->H0,2) * pow(lambda_G_mat/pba->lambda_G_mat_z_0,2)/ pow(a,3);
  ...
  if (pba->has_lambda == _TRUE_) {
    pvecback[pba->index_bg_rho_lambda] = rho_Lambda_at_z(pba,1/a-1);
  ...
  if (pba->has_idm_dr == _TRUE_) {
    pvecback[pba->index_bg_rho_idm_dr] = pba->Omega0_idm_dr * pow(pba->H0,2) * pow(lambda_G_mat/pba->lambda_G_mat_z_0,2) / pow(a,3);
...
int background_ncdm_init(
  ...
  for(k=0, filenum=0; k<pba->N_ncdm; k++) {
    ...
    pba->factor_ncdm[k]=pba->deg_ncdm[k]*4*_PI_*pow(pba->T_cmb*pba->T_ncdm[k]*_k_B_,4)*8*_PI_*_G_*pow(pba->lambda_G_mat_z_0,2)/3./pow(_h_P_/2./_PI_,3)/pow(_c_,7)*_Mpc_over_m_*_Mpc_over_m_;
  ...
  if (pba->background_verbose > 0) {
    ...
    if (pba->has_ncdm == _TRUE_) {
      ...
      for (n_ncdm=0;n_ncdm<pba->N_ncdm; n_ncdm++) {
        ...
        rho_nu_rel = 56.0/45.0*pow(_PI_,6)*pow(4.0/11.0,4.0/3.0)*_G_*pba->lambda_G_mat*pba->lambda_G_mat/pow(_h_P_,3)/pow(_c_,7)*
          pow(_Mpc_over_m_,2)*pow(pba->T_cmb*_k_B_,4); /**  We assume the interaction with the matter part*/
...
int background_derivs(
  ...
  dy[pba->index_bi_rs] = 1./a/H/sqrt(3.*(1.+3.*pba->beta_rho*pvecback[pba->index_bg_rho_b]*pow(pba->lambda_G_rad,2)/4./pvecback[pba->index_bg_rho_g]/pow(lambda_G_mat,2)))*sqrt(1.-pba->K*y[pba->index_bi_rs]*y[pba->index_bi_rs]); // TBC: curvature correction
