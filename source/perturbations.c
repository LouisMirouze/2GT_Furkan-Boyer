...
int perturbations_initial_conditions(struct precision * ppr,
  ...
  if (_scalars_) {
    ...
    rho_m_over_rho_r = rho_m*pow(pba->lambda_G_rad,2)/rho_r/pow(pba->lambda_G_mat,2);
...
int perturbations_derivs(double tau,
  ...
  R = 4./3./pba->beta_rho * pvecback[pba->index_bg_rho_g]*pow(lambda_G_mat,2)/pvecback[pba->index_bg_rho_b]/pow(pba->lambda_G_rad,2);
